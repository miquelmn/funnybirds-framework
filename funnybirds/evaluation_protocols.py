...
def accuracy_protocol(model, args, verbose=1):
    ...
    for samples in tqdm(test_loader, disable = (verbose < 1)):
        ...
        if args.gpu is not None:
            images = images.to(device)  # cuda(args.gpu, non_blocking=True)
            target = target.to(device)  # cuda(args.gpu, non_blocking=True)
...
def __handle_args(args, data, device):
    if args is not None and data is None:
        data = args.data
    elif args is None and data is None:
        raise ValueError
...
def __prepare_dataloader(data):
    transforms = None

    test_dataset = FunnyBirds(data, 'test', get_part_map=True, transform=transforms)
...
def controlled_synthetic_data_check_protocol(model, explainer, args=None, data=None, device=None, verbose=1):
    ...
    for samples in tqdm(test_loader, disable = (verbose < 1)):
        ...
        important_parts_for_thresholds = explainer.get_important_parts(images, part_maps, target,
                                                                       test_dataset.colors_to_part,
                                                                       thresholds=thresholds)
        for important_parts, threshold in zip(important_parts_for_thresholds, thresholds):
            minimal_sufficient_part_sets = test_dataset.get_minimal_sufficient_part_sets(
                target[0].item())
            max_J = 0
            for minimal_sufficient_part_set in minimal_sufficient_part_sets:
                minimal_sufficient_part_set = list(
                    map(lambda part_string: ''.join((x for x in part_string if x.isalpha())),
                        minimal_sufficient_part_set))
                J_current = len(
                    set(minimal_sufficient_part_set).intersection(set(important_parts))) / len(
                    minimal_sufficient_part_set)
                if J_current > max_J:
                    max_J = J_current
    ...
    for sample in tqdm(test_loader, disable = (verbose < 1)):
        ...
        for remove_part in bird_parts_keys:
            ...
            score[remove_part.split('_')[0]] = output[
                0, target].item()  # only keep part name, i.e. eye, instead of eye_model

        part_importances = explainer.get_part_importance(image, part_map, target,
                                                         test_dataset.colors_to_part)
        score_diffs = {}
        ...
        for key in score_diffs.keys():
            score_diffs_normalized.append(
                score_diffs[key])  # not necessary to normalize with spearmanr coefficient
            part_importances_normalized.append(
                part_importances[key])  # not necessary to normalize with spearmanr coefficient
...
def preservation_check_protocol(model, explainer, args=None, data=None, device=None, verbose=1):
    ...
    for samples in tqdm(test_loader, disable = (verbose < 1)):
        ...
        important_parts_for_thresholds = explainer.get_important_parts(images, part_maps,
...
def target_sensitivity_protocol(model, explainer, args=None, data=None, device=None, verbose=1):
    ...
    for sample in tqdm(test_loader, disable=(verbose < 1)):
        ...
        # get two classes that have each 2 parts in common with current target class, ie 3 parts
        # distance
        classes_w_two_overlap = test_dataset.get_classes_with_distance_n(target[0], 3)
        # get two classes out of these that don't have overlap in the two parts that overlap with
        # target class. E.g. one overlaps in foot and beak and the other in tail and wing
        found_classes = False
        ...
        # removing B parts should result in larger increase than removing A parts (its the same)
        ...
        drop_class1_when_rm_class1_parts = output_wo_parts_from_class1[0][class1] - output[0][
            class1]
        drop_class1_when_rm_class2_parts = output_wo_parts_from_class2[0][class1] - output[0][
            class1]

        drop_class2_when_rm_class1_parts = output_wo_parts_from_class1[0][class2] - output[0][
            class2]
        drop_class2_when_rm_class2_parts = output_wo_parts_from_class2[0][class2] - output[0][
            class2]

        # smaller because the drop should be more negative
        if not (
                drop_class1_when_rm_class1_parts < drop_class1_when_rm_class2_parts and
                drop_class2_when_rm_class2_parts < drop_class2_when_rm_class1_parts):
            number_assumption_wrong += 1
            continue

        assumption_strengths.append(
            drop_class1_when_rm_class2_parts.item() - drop_class1_when_rm_class1_parts.item())

        part_importances_class1 = explainer.get_part_importance(image, part_map, class1,
                                                                test_dataset.colors_to_part)
        part_importances_class2 = explainer.get_part_importance(image, part_map, class2,
                                                                test_dataset.colors_to_part)
...
def distractibility_protocol(model, explainer, args=None, data=None, device=None, verbose=1):
    ...
    for sample in tqdm(test_loader, disable=(verbose < 1)):
        ...
        for i in range(len(bg_object_ids)):
            image2 = test_dataset.get_background_intervention(class_idxs.squeeze(0).item(),
        ...
        for score_key in score.keys():
            ...
            if abs(score_diff) < abs(threshold_for_bg_importances) or original_score < score[
                score_key]:  # second condition: removing the part increases the class evidence
        ...
        explanation_important_parts_for_thresholds = explainer.get_important_parts(image, part_map,
                                                                                   target,
                                                                                   test_dataset.colors_to_part,
                                                                                   with_bg=True,
                                                                                   thresholds=thresholds)
        for explanation_important_parts, threshold in zip(
            ...
            J_current = len(
                set(explanation_important_parts).intersection(set(irrelevant_parts))) / len(
                irrelevant_parts)
...
def background_independence_protocol(model, args=None, data=None, device=None):
    ...
    for sample in tqdm(test_loader):
        ...
        for score_key in score.keys():
            ...
            if abs(score_diff) >= abs(threshold_for_bg_importances) and original_score > score[
                score_key]:
