...
class FunnyBirds(Dataset):
    ...
    def __init__(self, root_dir, mode, get_part_map=False, transform=None):
        ...
        self.colors_to_part = {(255, 255, 253): 'eye01', (255, 255, 254): 'eye02',
                               (255, 255, 0): 'beak', (255, 0, 1): 'foot01', (255, 0, 2): 'foot02',
                               (0, 255, 1): 'wing01', (0, 255, 2): 'wing02', (0, 0, 255): 'tail'}
        self.bg_color = (0, 0, 0)
    ...
    def __getitem__(self, idx):
        ...
        sample = {'image': image, 'params': params, 'class_idx': class_idx, 'part_map': part_map,
                  'image_idx': idx}
    ...
    def single_params_to_part_idxs(self, params_single):
        ...
        # parts_specification = {'beak': {'model': 'beak01.glb', 'color': 'yellow'},
        # 'eye': {'model': 'eye03.glb'}, 'foot': {'model': 'foot03.glb'}, 'tail': {'model':
        # 'tail02.glb', 'color': 'red'}, 'wing': {'model': 'placeholder', 'color': 'placeholder'}}
    ...
    def get_intervention(self, class_idx, image_idx, parts_removed):
        ...
        path = os.path.join(self.root_dir, self.mode + '_interventions', str(class_idx),
                            str(image_idx).zfill(6), image_name)

        return self.__get_interventions(path)

    def get_background_intervention(self, class_idx, image_idx, bg_object_id):
        image_name = str(bg_object_id) + '.png'
        path = os.path.join(self.root_dir, self.mode + '_interventions', str(class_idx),
                            str(image_idx).zfill(6), 'background_interventions', image_name)

        return self.__get_interventions(path)


    def __get_interventions(self, path):
        sample = {}
